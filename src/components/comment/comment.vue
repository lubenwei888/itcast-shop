<template>
  <div>
    ...
    <div class="comment">
      ...
      <!-- 加载更多 -->
      <p v-if="currentPageLength === 0 " class="nomore">没有更多了</p>
  ...
  methods: {
    ...
    async getCommentInfo () {
      const { data: res } = await this.$http.get(
        `/api/getcomments/${this.id ? this.id : this.goodsId}?pageindex=${this.page}`
      )
      if (res.status !== 0) this.$toast.fail('获取评论数据失败')
      // 获取每次当前页的评论内容的长度
      this.currentPageLength = res.message.length
      // 为防止每次请求的新数据将之前的数据覆盖
    ...
    // 判断内容是否为空
      ...
      const { data: res } = await this.$http.post(
        `/api/postcomment/${this.id ? this.id : this.goodsId}`,
        params
      )
